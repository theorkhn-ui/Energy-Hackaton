...
## Files
- `heatmap_monthly_ratio.png`: inverter × month peer ratio
...
`runs/plant_a/dashboard/index.html`: self-contained, dark theme, 65 clickable inverter tiles, per-inverter history + ticket markers.

## Open items
- Winter months are noisy (low irradiation), so consider irradiation-weighted ratios.
- The relative method cannot see plant-wide degradation. The absolute twin (`twin/TWIN.md`) addresses this.
