# Independent verification: Plant A headline statistics

Recomputed from raw data only (`data/raw/EP-Challenge-Final -/Plant A (start here)/`).
This pass does not reuse the main analysis code (`src/twin/*.py`) or its output files.
...
**Method (chosen independently for this check).** Daily energy per inverter = sum(P_AC) x 5/60 from the
...
| # | Claim | Result (primary) | Result (conservative) | Verdict |
|---|-------|--------------------|--------------------------|---------|
...
- "Median lead" is ambiguous; the 48-51 d match assumes lead = ticket date minus the *earliest*
...
quoted precisely). Claim 2's euro loss appears understated by ~30%, so quote "~EUR 500/yr" or
