...
the same timestamp). It is robust and curtailment-aware, but it is *relative*:
...
robust to the stale-kWp problem we found separately, and a uniform plant-wide
loss cannot hide in it.

**Health index** (daily): `HI[i,d] = Σ P_actual / Σ P_exp` over strong-sun
(`G > 300 W/m²`), non-curtailed steps. `HI = 1.0` → behaving like its healthy
...
~7–10%** versus the commissioned baseline (`twin_health_timeline.png`). Part is
...
- **Winter noise.** Few strong-sun steps in deep winter make HI noisier
  Nov–Feb; we smooth with a 75-day median for the timeline.
